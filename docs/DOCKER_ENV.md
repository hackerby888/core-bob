...
| `AUTOBAN` | `autoban` | `false` | Rotate out a discovered peer that fails most log requests (two 30s samples in a row) and keep its IP out of discovery: 10 min if it answers but has no logs, 4 h if it does not answer. Static `P2P_NODES` peers are never judged. |
